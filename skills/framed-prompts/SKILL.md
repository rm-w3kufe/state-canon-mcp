...
1. **A visible frame** — a delimiter the executor can copy verbatim, so the instruction is
   unmistakably separate from surrounding chatter. Nothing outside the frame is instruction.
2. **Context** — the minimum the executor needs and no session-private assumptions.
...
## The scar
The discipline was earned in the small print: an executor deployed to production reading state from
...
## Anti-patterns
- Instructions dribbled across several chat messages the executor must reassemble.
- "Fix the thing we discussed" — no self-contained scope.
- Omitting the stop-point, so the executor keeps going past the checkpoint.
