...
description: Use when an executor (agent or human) reports completed state-changing work — a deploy, a fix, a migration. Verify the claim against the live system before accepting it; never mark work done from the report alone.
---
...
2. For each claim, find the live observable that would prove it:
   ...
   - fixed → the failing symptom is absent in a fresh check, not in the executor's transcript
3. Check them yourself, through a channel independent of the executor. With a state RAG attached,
...
## The scar
An executor reported a fix "complete and verified." The target symptom was indeed gone — but the
